...
from custom_components.fluidra_pool.api_resilience import FluidraAuthError, FluidraConnectionError, FluidraError
...
async def test_user_flow_invalid_auth(hass: HomeAssistant, mock_api: AsyncMock) -> None:
    ...
    mock_api._cognito_initial_auth.side_effect = FluidraAuthError("401 Unauthorized")
...
async def test_user_flow_cannot_connect(hass: HomeAssistant, mock_api: AsyncMock) -> None:
    ...
    mock_api._cognito_initial_auth.side_effect = FluidraConnectionError("Connection timeout")
...
async def test_user_flow_unknown_error(hass: HomeAssistant, mock_api: AsyncMock) -> None:
    ...
    mock_api._cognito_initial_auth.side_effect = FluidraError("Something weird happened")
