...
async def async_setup_entry(
    ...
    for pool in pools:
        for device in pool["devices"]:
            ...
            # Speed select for variable speed pumps only
            if (
                device_type == "pump"
            ...
            # Schedule mode selects for pumps with schedules only
            if (
                device_type == "pump"
                and DeviceIdentifier.should_create_entity(device, "select")
...
class FluidraChlorinatorModeSelect(FluidraPoolControlEntity, SelectEntity):
    ...
    async def async_select_option(self, option: str) -> None:
        ...
        # Set optimistic option immediately for fast UI feedback
        self._optimistic_option = option
